...
class ToxicClassifier:
    ...
    def objective(self, trial):
        try:
            params = {
                'max_features': trial.suggest_int('max_features', 5000, 20000),  # Plage élargie
                'C': trial.suggest_float('C', 0.1, 10.0, log=True),
                'class_weight': trial.suggest_categorical('class_weight', ['balanced', None]),
                'max_iter': trial.suggest_int('max_iter', 100, 200)  # Plus d'itérations
            }
    ...
    def train(self, X_train, y_train, X_valid, y_valid, n_trials=10):
...
if __name__ == "__main__":
    # Configuration
    n_jobs = min(6, joblib.cpu_count() - 1)
    ...
    # Entraînement
    classifier = ToxicClassifier(n_jobs=n_jobs)
    ...
    classifier.save_model(data_dir / 'toxic_classifier.joblib') 